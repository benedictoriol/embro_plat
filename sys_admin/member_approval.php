...
$hasShopProfileCompletedAt = column_exists($pdo, 'shops', 'profile_completed_at');
    ...
    if ($id <= 0) {
        ...
        try {
            if ($action === 'approve_user') {
                ...
                $shopStatusStmt = $pdo->prepare("SELECT status, owner_id FROM shops WHERE id = ? FOR UPDATE");
                ...
                if (!$shopData) {
                    ...
                    $ownerId = (int) ($shopData['owner_id'] ?? 0);

                    $approveShopSql = "UPDATE shops SET status = 'active', rejection_reason = NULL, rejected_at = NULL";
                    if ($hasShopProfileCompletedAt) {
                        $approveShopSql .= ", profile_completed_at = COALESCE(profile_completed_at, NOW())";
                    }
                    $approveShopSql .= " WHERE id = ?";
                    $stmt = $pdo->prepare($approveShopSql);
                    $stmt->execute([$id]);
                    log_audit(
                        $pdo,
                        $actorId,
                ...
                if ($ownerId > 0) {
                        $ownerStatusStmt = $pdo->prepare("SELECT status FROM users WHERE id = ? FOR UPDATE");
                        ...
                        notify_owner_approval_decision($pdo, (int) $ownerId, true);
                    }
                ...
                if (empty($rejectionReason)) {
                    ...
                    $shopStmt = $pdo->prepare("SELECT status, owner_id, rejection_reason FROM shops WHERE id = ? FOR UPDATE");
                    $shopStmt->execute([$id]);
                    ...
                    if ($ownerId > 0) {
                            ...
                            notify_owner_approval_decision($pdo, (int) $ownerId, false, $rejectionReason);
                        }
...
$pendingShopsSelect = "
    SELECT s.id, s.shop_name, s.owner_id, s.address, s.phone, s.created_at,
        s.business_permit, s.permit_file,
        u.fullname as owner_name, u.email as owner_email";
if ($hasShopProfileCompletedAt) {
    $pendingShopsSelect .= ", s.profile_completed_at";
}
$pendingShopsSelect .= "
    FROM shops s
...
if ($hasShopProfileCompletedAt) {
    $pendingShopsSelect .= " AND s.profile_completed_at IS NOT NULL";
}
$pendingShopsSelect .= "
    ORDER BY s.created_at DESC
";
$pendingShops = $pdo->query($pendingShopsSelect)->fetchAll();
?>
<!DOCTYPE html>
<html lang="en">
<head>
