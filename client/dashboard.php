...
$stats_stmt = $pdo->prepare("
    SELECT
        ...
        SUM(CASE WHEN status IN ('pending', 'accepted', 'digitizing', 'production_pending', 'production', 'production_rework', 'qc_pending', 'ready_for_delivery', 'in_progress') THEN 1 ELSE 0 END) as active_orders,
...
$next_actions_stmt = $pdo->prepare("
    SELECT
        SUM(CASE WHEN (o.quote_status IN ('sent','issued','requested')
             OR EXISTS (SELECT 1 FROM order_quotes oq WHERE oq.order_id = o.id AND oq.status = 'sent')) THEN 1 ELSE 0 END) AS pending_quotes,
        SUM(CASE WHEN o.payment_status IN ('unpaid','partially_paid','pending_verification')
             AND o.status IN ('accepted','digitizing','production_pending','production','production_rework','qc_pending','ready_for_delivery','in_progress') THEN 1 ELSE 0 END) AS unpaid_orders,
        SUM(CASE WHEN EXISTS (SELECT 1 FROM design_approvals da WHERE da.order_id = o.id AND da.status IN ('pending','revision')) THEN 1 ELSE 0 END) AS pending_design_approvals,
        SUM(CASE WHEN EXISTS (SELECT 1 FROM order_fulfillments ofu WHERE ofu.order_id = o.id AND ofu.status IN ('ready_for_pickup','out_for_delivery')) THEN 1 ELSE 0 END) AS active_deliveries
    FROM orders o
    WHERE o.client_id = ?
");
$next_actions_stmt->execute([$client_id]);
$next_actions = $next_actions_stmt->fetch(PDO::FETCH_ASSOC) ?: [];

$delivery_tracking_stmt = $pdo->prepare("
    SELECT o.id, o.order_number, s.shop_name, ofu.status, ofu.courier, ofu.tracking_number, ofu.updated_at
    FROM order_fulfillments ofu
    JOIN orders o ON o.id = ofu.order_id
    JOIN shops s ON s.id = o.shop_id
    WHERE o.client_id = ?
      AND ofu.status IN ('ready_for_pickup','out_for_delivery')
    ORDER BY ofu.updated_at DESC
    LIMIT 4
");
$delivery_tracking_stmt->execute([$client_id]);
$delivery_tracking = $delivery_tracking_stmt->fetchAll(PDO::FETCH_ASSOC);


$shops_stmt = $pdo->query("
    SELECT s.id, s.shop_name, s.shop_description, s.rating, s.address, s.logo,
           sm.avg_rating, sm.completion_rate, sm.price_index, sm.cancellation_rate, sm.review_count, sm.availability_flag
    FROM shops s
    LEFT JOIN shop_metrics sm ON sm.shop_id = s.id
    WHERE s.status = 'active'
");
$featured_shops = compute_dss_ranked_shops($pdo, $shops_stmt->fetchAll(PDO::FETCH_ASSOC));
usort($featured_shops, function(array $a, array $b) {
    if (($b['dss_score'] ?? 0) === ($a['dss_score'] ?? 0)) {
        return ($b['rating'] ?? 0) <=> ($a['rating'] ?? 0);
    }
    return ($b['dss_score'] ?? 0) <=> ($a['dss_score'] ?? 0);
});
$featured_shops = array_slice($featured_shops, 0, 6);
    ...
    <div class="container">
        ...
        <div class="card">
            <div class="d-flex justify-between align-center">
                <div>
                    <h3>Next Actions</h3>
                    <p class="text-muted mb-0">Live operational items that need your attention.</p>
                </div>
                <a href="order_management.php" class="btn btn-outline-primary btn-sm">Open Orders</a>
            </div>
            <div class="stats-grid" style="margin-top: 14px;">
                <div class="stat-card">
                    <div class="stat-number"><?php echo (int) ($next_actions['pending_quotes'] ?? 0); ?></div>
                    <div class="stat-label">Pending Quotes</div>
                    <a href="pricing_quotation.php" class="btn btn-outline btn-sm mt-2">Review Quotes</a>
                </div>
                <div class="stat-card">
                    <div class="stat-number"><?php echo (int) ($next_actions['unpaid_orders'] ?? 0); ?></div>
                    <div class="stat-label">Unpaid / Partial Orders</div>
                    <a href="payment_handling.php" class="btn btn-outline btn-sm mt-2">Settle Payment</a>
                </div>
                <div class="stat-card">
                    <div class="stat-number"><?php echo (int) ($next_actions['pending_design_approvals'] ?? 0); ?></div>
                    <div class="stat-label">Design Approvals Needed</div>
                    <a href="design_proofing.php" class="btn btn-outline btn-sm mt-2">Approve Designs</a>
                </div>
                <div class="stat-card">
                    <div class="stat-number"><?php echo (int) ($next_actions['active_deliveries'] ?? 0); ?></div>
                    <div class="stat-label">Delivery Tracking</div>
                    <a href="track_order.php" class="btn btn-outline btn-sm mt-2">Track Orders</a>
                </div>
            </div>
            <?php if(!empty($delivery_tracking)): ?>
                <div class="table-responsive" style="margin-top: 16px;">
                    <table class="table">
                        <thead><tr><th>Order</th><th>Shop</th><th>Status</th><th>Tracking</th><th></th></tr></thead>
                        <tbody>
                        <?php foreach($delivery_tracking as $item): ?>
                            <tr>
                                <td>#<?php echo htmlspecialchars($item['order_number']); ?></td>
                                <td><?php echo htmlspecialchars($item['shop_name']); ?></td>
                                <td><?php echo ucfirst(str_replace('_', ' ', (string) $item['status'])); ?></td>
                                <td><?php echo !empty($item['tracking_number']) ? htmlspecialchars($item['tracking_number']) : '—'; ?></td>
                                <td><a href="track_order.php?order_id=<?php echo (int) $item['id']; ?>" class="btn btn-sm btn-outline-primary">View</a></td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
                    </table>
                    ...
                    <?php foreach($featured_shops as $shop): ?>
                          <div class="shop-card" data-href="shop_details.php?shop_id=<?php echo (int) $shop['id']; ?>">
                            ...
                            <div class="shop-rating">
                                ...
                                DSS <?php echo number_format(((float) ($shop['dss_score'] ?? 0)) * 100, 1); ?>%
                            </div>
