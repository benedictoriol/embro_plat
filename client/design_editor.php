    ...
    <div class="container">
        ...
        <div class="editor-layout">
            ...
            <div class="editor-panel">
                ...
                <div class="help-grid">
                    <div class="help-item"><strong>Auto-save:</strong> Draft state is cached locally every 10 minutes. Version history is stored in the backend.</div>
...
const draftStorageKey = 'embroider_design_editor_draft';
const orderIdFromUrl = new URLSearchParams(window.location.search).get('order_id');
const activeOrderId = orderIdFromUrl && Number(orderIdFromUrl) > 0 ? Number(orderIdFromUrl) : null;
...
function applyDesignPayload(payload) {
    state.elements = normalizeElements(payload.elements || []);
    state.hoopPreset = payload.hoopPreset || 'M';
    state.threadColor = payload.threadColor || '#111827';
    state.showSafeArea = payload.showSafeArea !== false;
    state.canvasType = getSupportedCanvasType(payload.canvasType);
    state.canvasColor = normalizeCanvasColor(payload.canvasColor);
    state.placementMethod = payload.placementMethod || 'center-chest';
    state.selectedId = null;
    hoopPreset.value = state.hoopPreset;
    threadColor.value = state.threadColor;
    safeAreaToggle.value = state.showSafeArea ? 'on' : 'off';
...
async function fetchServerVersions() {
    const params = new URLSearchParams({ versions: '1' });
    if (activeOrderId) {
        params.set('order_id', String(activeOrderId));
    }
    const response = await fetch(`../api/design_api.php?${params.toString()}`);
    if (!response.ok) {
        throw new Error('Failed to load design versions.');
    }
    const payload = await response.json();
    state.versions = Array.isArray(payload.data) ? payload.data : [];
    renderVersions();
}

async function loadServerVersion(versionId) {
    ...
    const response = await fetch(`../api/design_api.php?${params.toString()}`);
    if (!response.ok) {
        throw new Error('Failed to load selected version.');
    }
    const payload = await response.json();
    const design = payload.data || null;
    if (!design || !design.design_json) {
        throw new Error('Design payload is empty.');
    }
    applyDesignPayload(JSON.parse(design.design_json));
}
...
function renderVersions() {
    ...
    state.versions.forEach(version => {
        ...
        card.innerHTML = `
            <span>v${version.version_number} - ${version.design_name || 'Untitled Design'}</span>
            <div class="version-actions">
                <button class="btn btn-outline btn-sm" data-action="load">Load</button>
            </div>
        `;

        card.querySelector('[data-action="load"]').onclick = async () => {
            try {
                await loadServerVersion(version.id);
            } catch (error) {
                alert(error.message || 'Unable to load this version.');
            }
        };
...
saveVersionBtn.addEventListener('click', async () => {
    try {
        const response = await fetch('../api/design_api.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                order_id: activeOrderId,
                design_name: 'Editor Version',
                design_json: {
                    elements: state.elements,
                    hoopPreset: state.hoopPreset,
                    threadColor: state.threadColor,
        ...
        const payload = await response.json();
        if (!response.ok) {
            throw new Error(payload.error || 'Failed to save design version.');
        }
    await fetchServerVersions();
    } catch (error) {
        alert(error.message || 'Unable to save design version.');
    }
});
