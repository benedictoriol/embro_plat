...
$summary_stmt = $pdo->prepare("
    SELECT
        ...
        SUM(CASE WHEN o.payment_release_status = 'released' THEN 1 ELSE 0 END) AS released_orders,
        SUM(CASE WHEN o.payment_status = 'pending_verification' THEN 1 ELSE 0 END) AS pending_orders,
        SUM(CASE WHEN o.payment_status IN ('unpaid', 'failed') THEN 1 ELSE 0 END) AS action_needed,
...
function payment_badge_class($status)
{
    return match ($status) {
        'paid' => 'badge-success',
        'partially_paid', 'pending_verification' => 'badge-warning',
    ...
    <div class="dashboard-header fade-in">
        ...
        <div class="card">
            ...
            <div class="stat-grid">
                ...
                <div class="stat-item">
                    <small class="text-muted">Released to Shop</small>
                    <h4><?php echo (int) ($summary['released_orders'] ?? 0); ?></h4>
                </div>
                <div class="stat-item">
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge <?php echo payment_badge_class($payment['status'] ?? 'pending'); ?>">
                                        <?php echo htmlspecialchars(payment_status_label((string) ($payment['status'] ?? 'pending_verification'))); ?>
                                    </span>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="action-row">
                                        ...
                                        <?php if (normalize_payment_status((string) ($payment['status'] ?? '')) === 'paid'): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge <?php echo payment_badge_class($order['payment_status']); ?>">
                                        <?php echo htmlspecialchars(payment_status_label((string) $order['payment_status'])); ?>
                                    </span>
                                </td>
                                <td>
