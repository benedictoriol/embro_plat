...
$proofing_active_statuses = order_workflow_active_statuses();
$proofing_status_placeholders = order_workflow_status_placeholders($proofing_active_statuses);
...
function resolve_design_preview_url(?string $preview_file): ?string {
    if(!$preview_file) {
        return null;
    }

    if(str_starts_with($preview_file, 'data:image/')) {
        return $preview_file;
    }

    return proof_file_url($preview_file);
}
...
function estimate_design_quote_from_image(?string $image_path, string $service_type, array $shop_pricing_settings, ?int $fallback_width = null, ?int $fallback_height = null, ?int $fallback_colors = null, int $quantity = 1): array {
    ...
    $computedWidthMm = px_to_mm_estimate($width);
    $computedHeightMm = px_to_mm_estimate($height);
    ...
    $pricing = calculate_embroidery_quote([
        ...
        'width_mm' => $computedWidthMm,
        'height_mm' => $computedHeightMm,
...
if($prefill_order_id > 0) {
    $prefill_stmt = $pdo->prepare("SELECT o.id, o.order_number, o.shop_id, o.service_type, o.design_description, o.design_file, o.width_px, o.height_px, o.detected_width_mm, o.detected_height_mm, o.fits_cap_area, o.suggested_width_mm, o.suggested_height_mm, o.scale_ratio, o.client_notes, s.shop_name,
        dv.preview_image_path as design_version_preview
        FROM orders o
        JOIN shops s ON s.id = o.shop_id
        LEFT JOIN saved_designs dv ON dv.id = o.design_version_id
...
if(isset($_POST['request_quote'])) {
    ...
    $imported_design_payload_raw = trim((string) ($_POST['imported_design_payload'] ?? ''));
    $imported_design_payload = [];
    if($imported_design_payload_raw !== '') {
        $decoded_import_payload = json_decode($imported_design_payload_raw, true);
        if(is_array($decoded_import_payload)) {
            $imported_design_payload = $decoded_import_payload;
        }
    }
    ...
    if($error === '' && !$uploaded_design_file && $customize_order_id > 0) {
        $customized_stmt = $pdo->prepare("SELECT o.id, o.design_file, o.width_px, o.height_px, o.detected_width_mm, o.detected_height_mm, o.fits_cap_area, o.suggested_width_mm, o.suggested_height_mm, o.scale_ratio,
            dv.preview_image_path as design_version_preview
            FROM orders o
            LEFT JOIN saved_designs dv ON dv.id = o.design_version_id
        ...
        if($customized_order) {
            ...
            $custom_design_version_preview = trim((string) ($customized_order['design_version_preview'] ?? ''));
            if($uploaded_design_file === null && $custom_design_version_preview !== '' && empty($imported_design_payload['design_preview'])) {
                $imported_design_payload['design_preview'] = $custom_design_version_preview;
            }
    ...
    if($error === '') {
        ...
        $quote_details = [
            ...
            'imported_design' => !empty($imported_design_payload) ? $imported_design_payload : null,
        ...
        $order_id = order_create_unified_request($pdo, [
            ...
            'design_description' => $design_description,
            'quantity' => 1,
            'price' => (float) ($design_estimate['estimated_price'] ?? 0),
            'client_notes' => 'Quote request submitted via Services page.',
            'quote_details' => json_encode($quote_details),
            'design_file' => $uploaded_design_file,
            'design_approved' => 0,
        ...
        if (function_exists('log_audit')) {
            log_audit(
                ...
                'quote_request_generated',
                'orders',
                $order_id,
                [],
                [
                    'system_suggested_price' => (float) ($design_estimate['estimated_price'] ?? 0),
                    'pricing_breakdown' => $design_estimate['price_components'] ?? [],
                ]
            );
        }

        record_order_status_history_once(
            $pdo,
            $order_id,
            STATUS_PENDING,
            order_workflow_display_progress(STATUS_PENDING, 0),
...
$selected_custom_order_preview_url = resolve_design_preview_url($selected_custom_order['design_file'] ?? null);
if($selected_custom_order_preview_url === null) {
    $selected_custom_order_preview_url = resolve_design_preview_url($selected_custom_order['design_version_preview'] ?? null);
}
...
if(isset($_POST['request_revision'])) {
    ...
    if(!$approval) {
        ...
        notify_shop_staff($pdo, (int) $approval['shop_id'], $order_id, 'order_status', $message);
        messaging_auto_thread_on_client_owner_discussion($pdo, $order_id, $client_id, 'Revision requested: ' . $revision_notes);
...
if(isset($_POST['reject_proof'])) {
    ...
    if(!$approval) {
       ...
       if(!empty($approval['approval_id'])) {
        ...
        notify_shop_staff($pdo, (int) $approval['shop_id'], $order_id, 'order_status', $message);
        messaging_auto_thread_on_client_owner_discussion($pdo, $order_id, $client_id, 'Proof rejected: ' . $rejection_notes);
    ...
    if(!$order_quote) {
       ...
       $success = 'Price quote accepted. This request is now an official order and the shop has been notified.';
        ...
        if($comment === '') {
            ...
            messaging_auto_thread_on_client_owner_discussion($pdo, $order_id, $client_id, 'Price quote rejected: ' . $comment);
        ...
        if($comment === '') {
            ...
            messaging_auto_thread_on_client_owner_discussion($pdo, $order_id, $client_id, 'Negotiation requested: ' . $comment);
        ...
        messaging_auto_thread_on_client_owner_discussion($pdo, $order_id, $client_id, 'Client declined current shop quote.');
...
$approvals_stmt = $pdo->prepare("
    SELECT o.id as order_id, o.order_number, o.status as order_status, o.design_approved,
           ...
           dv.version_number as design_version_no, dv.preview_image_path as design_version_preview,
           dv.created_at as design_version_created_at, dv.design_name as design_project_title
    FROM orders o
    JOIN shops s ON o.shop_id = s.id
    LEFT JOIN design_approvals da ON da.order_id = o.id
    LEFT JOIN saved_designs dv ON dv.id = o.design_version_id
    WHERE o.client_id = ?
      AND o.status IN ($proofing_status_placeholders)
...
");
$approvals_stmt->execute(array_merge([$client_id], $proofing_active_statuses));
...
");
$requests_stmt->execute(array_merge([$client_id], $proofing_active_statuses));
...
foreach($request_history as &$request) {
    ...
    $imported_design = is_array($quote_details['imported_design'] ?? null) ? $quote_details['imported_design'] : [];
    $request['imported_design_preview_url'] = resolve_design_preview_url($imported_design['design_preview'] ?? null);
    $request['design_version_preview_url'] = resolve_design_preview_url($request['design_version_preview'] ?? null);
...
<body>
   <?php require_once __DIR__ . '/includes/customer_navbar.php'; ?>
    <div class="container">
            ...
            <div class="card">
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="hidden" name="imported_design_payload" id="importedDesignPayloadInput" value="">
            ...
            <div class="d-flex flex-column gap-3">
                <?php if($selected_custom_order): ?>
                    <div class="service-note">
                        ...
                        <?php if(!empty($selected_custom_order_preview_url)): ?>
                            <div class="request-design-preview">
                                <p class="mb-1"><strong>Selected customized design preview:</strong></p>
                                <?php if(is_design_image($selected_custom_order_preview_url)): ?>
                                    <img src="<?php echo htmlspecialchars($selected_custom_order_preview_url); ?>" alt="Customized design preview for order <?php echo htmlspecialchars($selected_custom_order['order_number']); ?>">
                                <?php else: ?>
                                    <a class="btn btn-sm btn-outline mt-2" href="<?php echo htmlspecialchars($selected_custom_order_preview_url); ?>" target="_blank" rel="noopener">
                                        <i class="fas fa-eye"></i> View customized design file
                                    </a>
                                <?php endif; ?>
                            </div>
                        <?php endif; ?>
                ...
                <div class="request-history">
                    <?php foreach($request_history as $request): ?>
                        <div class="request-item">
                            ...
                            <?php
                                $history_design_preview_url = $request['design_file_url'] ?: ($request['imported_design_preview_url'] ?: $request['design_version_preview_url']);
                                $history_design_is_image = is_design_image($history_design_preview_url);
                            ?>
                            <?php if(!empty($history_design_preview_url)): ?>
                                <div class="request-design-preview">
                                    <p class="mb-0"><strong>Design preview:</strong></p>
                                    <?php if($history_design_is_image): ?>
                                        <img src="<?php echo htmlspecialchars($history_design_preview_url); ?>" alt="Design preview for order <?php echo htmlspecialchars($request['order_number']); ?>">
                                    <?php else: ?>
                                        <a class="btn btn-sm btn-outline mt-2" href="<?php echo htmlspecialchars($history_design_preview_url); ?>" target="_blank" rel="noopener">
                                            <i class="fas fa-file-arrow-down"></i> View design file
                                        </a>
                                    <?php endif; ?>
                                </div>
                            <?php endif; ?>
        ...
        const importedDesignPayloadInput = document.getElementById('importedDesignPayloadInput');
        ...
        (function hydrateProofingDraftFromDesignEditor() {
            ...
            try {
                ...
                if (importedDesignPayloadInput) {
                    importedDesignPayloadInput.value = JSON.stringify({
                        source: draft.source || 'design_editor',
                        title: draft.title || '',
                        design_summary: draft.design_summary || '',
                        design_details: draft.design_details || {},
                        stitch_estimate: draft.stitch_estimate || {},
                        estimated_price: Number(draft.estimated_price || 0),
                        estimated_price_label: draft.estimated_price_label || '',
                        estimate_note: draft.estimate_note || '',
                        design_preview: draft.design_preview || '',
