...
$allow_cod = (bool) system_setting_get($pdo, 'payment', 'allow_cod', true);
foreach (['gcash', 'card', 'cod', 'pickup'] as $method_code) {
    $pref = $payment_preferences[$method_code] ?? [];
    if ((int) ($pref['is_enabled'] ?? 0) === 1) {
        $allowed_payment_methods[] = $method_code;
    }
    if ((int) ($pref['is_default'] ?? 0) === 1) {
        $default_payment_method = $method_code;
    }
}
if (empty($allowed_payment_methods)) {
    $allowed_payment_methods = $allow_cod ? ['cod', 'pickup'] : ['pickup'];
}

if (!$allow_cod) {
    $allowed_payment_methods = array_values(array_filter($allowed_payment_methods, fn($method) => $method !== 'cod'));
    if (empty($allowed_payment_methods)) {
        $allowed_payment_methods = ['pickup'];
    }
}
if ($default_payment_method === '' || !in_array($default_payment_method, $allowed_payment_methods, true)) {
    $default_payment_method = $allowed_payment_methods[0] ?? 'cod';
}
$available_services = [
    'T-shirt Embroidery',
    'Logo Embroidery',
...
$system_min_order_quantity = max(1, (int) system_setting_get($pdo, 'business_rules', 'min_order_quantity', 1));
$system_max_order_quantity = max($system_min_order_quantity, (int) system_setting_get($pdo, 'business_rules', 'max_order_quantity', 1000));
...
$shops = array_map(function($shop) use ($search_term, $service_filter) {
    $distance_score = calculate_distance_score($search_term, $shop);
    $service_match = $service_filter !== ''
        ? (in_array($service_filter, $shop['service_list'], true) ? 1.0 : 0.0)
        : 1.0;

    // Keep DSS as the primary recommendation score and expose transparent component values.
    $shop['ranking_score'] = round(((float) ($shop['dss_score'] ?? 0)) * 100, 1);
    $shop['ranking_breakdown'] = [
        'dss' => round(((float) ($shop['dss_score'] ?? 0)) * 100, 1),
        'distance' => round($distance_score * 100, 1),
        'rating' => round(((float) ($shop['dss_breakdown']['rating'] ?? 0)) * 100, 1),
        'completion' => round(((float) ($shop['dss_breakdown']['completion'] ?? 0)) * 100, 1),
        'pricing' => round(((float) ($shop['dss_breakdown']['price'] ?? 0)) * 100, 1),
        'availability' => round(((float) ($shop['dss_breakdown']['availability'] ?? 0)) * 100, 1),
        'specialization' => round($service_match * 100, 1),
    ];
    return $shop;
}, $shops);
...
if(isset($_POST['place_order'])) {
    ...
    try {
        ...
        if (!in_array($payment_method, $allowed_payment_methods, true)) {
            throw new RuntimeException('Selected payment method is not available in your profile payment preferences.');
        }

        if ($payment_method !== 'pickup' && !$default_address) {
            throw new RuntimeException('Please save a default delivery address in your profile before placing this order.');
        }

        $delivery_profile_snapshot = $default_address ? [
            'address_id' => (int) ($default_address['id'] ?? 0),
            'label' => $default_address['label'] ?? null,
            'recipient_name' => $default_address['recipient_name'] ?? null,
            'phone' => $default_address['phone'] ?? null,
            'country' => $default_address['country'] ?? null,
            'province' => $default_address['province'] ?? null,
            'city' => $default_address['city'] ?? null,
            'barangay' => $default_address['barangay'] ?? null,
            'street_address' => $default_address['street_address'] ?? null,
            'address_line2' => $default_address['address_line2'] ?? null,
            'postal_code' => $default_address['postal_code'] ?? null,
        ] : new stdClass();
        ...
        if ($quantity < $system_min_order_quantity || $quantity > $system_max_order_quantity) {
            throw new RuntimeException('Quantity must be between ' . $system_min_order_quantity . ' and ' . $system_max_order_quantity . '.');
        }
        ...
        $quote_details = [
            ...
            'delivery_profile' => $delivery_profile_snapshot,
        ...
        $order_id = order_create_unified_request($pdo, [
            ...
            'quote_details' => $quote_details_json,
        ...
        notify_business_event($pdo, 'order_submitted', (int) $order_id, ['actor_id' => $client_id]);
            ...
            <div class="card mb-4">
                ...
                <div class="row mt-2" style="display: flex; gap: 15px; flex-wrap: wrap;">
                    <div class="form-group" style="flex: 1; min-width: 180px;">
                        ...
                        <input type="number" name="quantity" id="sampleQuantityInput" class="form-control" value="<?php echo $system_min_order_quantity; ?>" min="<?php echo $system_min_order_quantity; ?>" max="<?php echo $system_max_order_quantity; ?>" required>
                    </div>
                        ...
                        <div class="d-flex align-center">
                            ...
                            <div>
                                ...
                                <div class="ranking-meta">
                                    ...
                                    <span>DSS: <?php echo $shop['ranking_breakdown']['dss']; ?>%</span>
                                    <span>Ratings: <?php echo $shop['ranking_breakdown']['rating']; ?>%</span>
                                    <span>Completion: <?php echo $shop['ranking_breakdown']['completion']; ?>%</span>
                                    <span>Availability: <?php echo $shop['ranking_breakdown']['availability']; ?>%</span>
            ...
            <div class="card mb-4">
                ...
                <div class="row" style="display: flex; gap: 15px;">
                    <div class="form-group" style="flex: 1;">
                        ...
                        <input type="number" name="quantity" class="form-control" value="<?php echo $system_min_order_quantity; ?>" min="<?php echo $system_min_order_quantity; ?>" max="<?php echo $system_max_order_quantity; ?>" required>
                    </div>
            ...
            <div class="card mb-4">
                ...
                <div class="form-group">
                    ...
                    <div class="row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 10px;">
                        ...
                        foreach ($payment_method_labels as $method_code => $method_label):
                            $enabled = in_array($method_code, $allowed_payment_methods, true);
                        ?>
                            <label class="payment-method-option">
                                <input type="radio" name="payment_method" value="<?php echo htmlspecialchars($method_code); ?>" <?php echo $enabled ? '' : 'disabled'; ?> <?php echo $method_code === $default_payment_method ? 'checked' : ''; ?> <?php echo $enabled ? 'required' : ''; ?>>
                                <span><strong><?php echo htmlspecialchars($method_label); ?></strong></span>
                                <?php if (!$enabled): ?>
                                    <small class="text-muted d-block">Disabled in your payment preferences.</small>
                                <?php endif; ?>
                            </label>
                        <?php endforeach; ?>
                    </div>
                    <small class="text-muted d-block mt-2">Payment options here come from your customer profile preferences.</small>
                </div>
                <div class="form-group mb-0">
                    <label>Delivery Address</label>
                    <div class="d-flex align-center" style="gap: 10px; flex-wrap: wrap;">
                        <?php if ($default_address): ?>
                            <span class="text-muted"><?php echo htmlspecialchars(trim(($default_address['street_address'] ?? '') . ', ' . ($default_address['barangay'] ?? '') . ', ' . ($default_address['city'] ?? '') . ', ' . ($default_address['province'] ?? ''))); ?></span>
                        <?php else: ?>
                            <span class="text-muted">No default delivery address found.</span>
                        <?php endif; ?>
                        <a href="customer_profile.php#delivery-address" class="btn btn-outline-primary btn-sm">Manage Delivery Address</a>
                    </div>
                </div>
            </div>
