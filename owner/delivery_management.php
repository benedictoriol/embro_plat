...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_fulfillment'])) {
    ...
    $order_stmt = $pdo->prepare("
        ...
        WHERE o.id = ? AND o.shop_id = ? AND o.status IN ('ready_for_delivery', 'delivered', 'completed')
    ");
    $order_stmt->execute([$order_id, $shop_id]);
    $order = $order_stmt->fetch();

    if(!$order) {
        $error = 'Only ready-for-delivery or completed orders can be moved to delivery or pickup.';
    ...
    } elseif(!isset($delivery_method_options[$delivery_method])) {
        ...
        [$saved, $save_error] = FulfillmentService::save(
            $pdo,
            $order,
            [
...
$orders_stmt = $pdo->prepare("
    ...
    LEFT JOIN order_fulfillments f ON f.order_id = o.id
     WHERE o.shop_id = ?
      AND (
          o.status IN ('ready_for_delivery', 'delivered', 'completed')
    ...
    <div class="container" style="margin-top: 30px;">
        ...
        <?php else: ?>
            <?php foreach($orders as $order): ?>
                <?php $payment_hold = payment_hold_status(STATUS_COMPLETED, $order['payment_status'] ?? 'unpaid', $order['payment_release_status'] ?? null); ?>
                ...
                <div class="delivery-card">
                    ...
                    <div class="delivery-meta">
                        ...
                        <div><strong>Delivery method:</strong> <?php echo htmlspecialchars($delivery_method_options[$order['delivery_method'] ?? ''] ?? 'Not set'); ?></div>
                        ...
                        <div><strong>Shipped at:</strong> <?php echo $order['shipped_at'] ? date('M d, Y H:i', strtotime($order['shipped_at'])) : '—'; ?></div>
                    ...
                    <form method="POST" class="form-grid">
                        ...
                        <div>
                            ...
                            <select name="delivery_method" class="form-control" required>
                                <?php foreach($delivery_method_options as $value => $label): ?>
                                    <option value="<?php echo htmlspecialchars($value); ?>" <?php echo ($order['delivery_method'] ?? ($order['fulfillment_type'] === 'pickup' ? 'store_pickup' : 'courier_delivery')) === $value ? 'selected' : ''; ?>>
