...
if($user_role === 'owner') {
    ...
    $shop_stmt = $pdo->prepare("
        ...
        WHERE ss.user_id = ?
          ...
          AND (ss.staff_role = 'hr' OR LOWER(REPLACE(ss.position, ' ', '_')) = 'hr_staff')
        ORDER BY ss.id DESC
        LIMIT 1
    " );
    $shop_stmt->execute([$user_id]);
...
if(function_exists('update_order_estimated_completion')) {
    ...
    $eta_refresh_stmt = $pdo->prepare("SELECT estimated_completion FROM orders WHERE id = ? LIMIT 1");
    $eta_refresh_stmt->execute([$order_id]);
    $order['estimated_completion'] = $eta_refresh_stmt->fetchColumn() ?: null;
}
...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['assign_order'])) {
    ...
    if($assign_order_id !== $order_id) {
        ...
        [$assigned_ok, $assigned_error] = OrderService::assignStaff($pdo, $order_id, $staff_id, $user_id);
        if($assigned_ok) {
            ...
            $error = $assigned_error ?: 'Failed to assign this order to the selected staff member.';
        }
    } else {
        $expected_assignee = isset($order['assigned_to']) ? (int) $order['assigned_to'] : null;
        [$unassign_ok, $unassign_error] = OrderService::unassignStaff($pdo, $order_id, (int) $shop['id'], $expected_assignee);
        if($unassign_ok) {
            $order['assigned_to'] = null;
            $order['assigned_name'] = null;
            $success = 'Order unassigned.';
        } else {
            $error = $unassign_error ?: 'Assignment changed by another user. Please refresh and try again.';
        }
    }
        ...
        if(!empty($availability_days) && !in_array($schedule_day, $availability_days, true)) {
            ...
            if($max_active_orders > 0 && $scheduled_count >= $max_active_orders) {
                ...
                if(empty($error)) {
                    ...
                    $assignment_saved = assign_order_to_staff($pdo, $schedule_order_id, $staff_id, $user_id);
                    if(!$assignment_saved) {
                    ...
                    if(function_exists('update_order_estimated_completion')) {
                        update_order_estimated_completion($pdo, $schedule_order_id);
                    }
            ...
            <div class="detail-group">
                ...
                <p><strong>Estimated Completion:</strong>
                    <?php if(!empty($order['estimated_completion'])): ?>
                        <?php echo date('M d, Y h:i A', strtotime($order['estimated_completion'])); ?>
                        <span class="text-muted">(estimate only)</span>
                    <?php else: ?>
