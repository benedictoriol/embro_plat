...
$production_tracking_statuses = [STATUS_ACCEPTED, STATUS_PRODUCTION_PENDING, STATUS_PRODUCTION, STATUS_PRODUCTION_REWORK, STATUS_IN_PROGRESS];
$production_tracking_placeholders = order_workflow_status_placeholders($production_tracking_statuses);
...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['start_production'])) {
    ...
    [$started, $start_error, $thread_log] = ProductionService::startOrderProduction(
        $pdo,
        $order_id,
        (int) ($shop['id'] ?? 0),
        $owner_id,
    ...
    if(!$started) {
        $error = $start_error ?: 'Unable to start production.';
    } else {
        $order_number_stmt = $pdo->prepare("SELECT order_number FROM orders WHERE id = ? LIMIT 1");
        $order_number_stmt->execute([$order_id]);
        $order_number = (string) ($order_number_stmt->fetchColumn() ?: $order_id);

    $success = 'Production has started for order #' . $order_number . '.';
        if(is_array($thread_log) && ($thread_log['mode'] ?? '') === 'reserved_consumption') {
            $success .= ' Used previously reserved materials for this order.';
        } elseif(is_array($thread_log) && !empty($thread_log['thread']['already_logged'])) {
            $success .= ' Thread consumption was already logged earlier.';
        } elseif(is_array($thread_log) && isset($thread_log['thread']['required_qty_m'])) {
            $success .= ' Deducted ' . number_format((float) $thread_log['thread']['required_qty_m'], 2) . ' m of thread from inventory.';
        }
    }
}
...
$orders_stmt = $pdo->prepare("
    ...
    WHERE o.shop_id = ? AND o.status IN ($production_tracking_placeholders)
    ORDER BY o.created_at DESC
");
$orders_stmt->execute(array_merge([$shop['id'] ?? 0], $production_tracking_statuses));
$orders = $orders_stmt->fetchAll();
$exception_summary_map = [];
if(function_exists('order_exception_summaries') && !empty($orders)) {
    $exception_summary_map = order_exception_summaries($pdo, array_column($orders, 'id'));
}
                ...
                <div class="proof-grid">
                    <?php foreach($orders as $order): ?>
                        <?php
                            ...
                            $payment_hold = payment_hold_status($order['status'] ?? STATUS_PENDING, $order['payment_status'] ?? 'unpaid', $order['payment_release_status'] ?? null);
                            $exception_summary = $exception_summary_map[(int) ($order['id'] ?? 0)] ?? ['open_count' => 0, 'escalated_count' => 0, 'has_blocking' => false];
                        ?>
                        <div class="proof-card">
                            ...
                            <div class="d-flex gap-2 align-center">
                                ...
                                <?php if((int) ($exception_summary['open_count'] ?? 0) > 0): ?>
                                    <span class="badge badge-<?php echo !empty($exception_summary['has_blocking']) ? 'danger' : 'warning'; ?>">
                                        Exceptions: <?php echo (int) $exception_summary['open_count']; ?>
                                    </span>
                                <?php endif; ?>
                            </div>
