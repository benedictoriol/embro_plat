...
$opened_shop_id = (int) ($_SESSION['user']['shop_id'] ?? 0);

if ($opened_shop_id > 0) {
    $shop_stmt = $pdo->prepare("SELECT id, shop_name FROM shops WHERE id = ? AND owner_id = ?");
    $shop_stmt->execute([$opened_shop_id, $owner_id]);
} else {
    $shop_stmt = $pdo->prepare("SELECT id, shop_name FROM shops WHERE owner_id = ? ORDER BY id DESC LIMIT 1");
...
$materials_stmt = $pdo->prepare("SELECT id, name, unit, current_stock, min_stock_level FROM raw_materials WHERE shop_id = ? AND status = 'active' ORDER BY name");
