...
if($order['status'] !== 'pending') {
        ...
        [$cancel_ok, $cancel_error] = automation_transition_order_status(
            $pdo,
            $order_id,
            STATUS_CANCELLED,
            $owner_id,
            $owner_role,
            'Owner rejected order: ' . $rejection_reason,
            false,
            'reject_order'
        );

        if(!$cancel_ok) {
            $error = $cancel_error ?: 'Unable to reject this order from its current status.';
        } else {
            ...
            record_order_progress_log_once($pdo, $order_id, STATUS_CANCELLED, 'Owner rejected order', $rejection_reason, $owner_id);
        ...
        notify_business_event($pdo, 'order_rejected', $order_id, [
        ...
        notify_dispute_update($pdo, $order_id, 'Order was rejected by shop owner. Reason: ' . $rejection_reason, $owner_id);
