...
$profile_completed_column_exists = false;
try {
    $profile_completed_column_stmt = $pdo->query("SHOW COLUMNS FROM shops LIKE 'profile_completed_at'");
    $profile_completed_column_exists = (bool) $profile_completed_column_stmt->fetch();
} catch (PDOException $e) {
    $profile_completed_column_exists = false;
}

$pending_notice = $_SESSION['owner_gate_notice'] ?? '';
unset($_SESSION['owner_gate_notice']);

$onboarding_snapshot = fetch_owner_onboarding_snapshot((int) $owner_id, $pdo);
if ($pending_notice === '' && in_array($onboarding_snapshot['state'] ?? '', ['awaiting_approval', 'rejected'], true)) {
    $pending_notice = ($onboarding_snapshot['state'] ?? '') === 'rejected'
        ? 'Your shop has not yet been verified. Please update your shop profile details and resubmit for admin re-evaluation.'
        : 'Your owner account is pending approval. Complete your shop profile and wait for admin approval to unlock the full owner dashboard.';
}
    ...
    try {
        if ($action === 'submit_business_information') {
           $individual_registered_name = $business_form_values['individual_registered_name'];
            ...
            if ($profile_completed_column_exists) {
                $update_sql .= ", profile_completed_at = NOW()";
            }
            ...
            $owner_status_stmt = $pdo->prepare("UPDATE users SET status = 'pending' WHERE id = ?");
            $owner_status_stmt->execute([$owner_id]);

            $shop_stmt->execute([$owner_id]);
            $shop = $shop_stmt->fetch();
            
            $adminNotification = sprintf(
                'Shop profile submitted for review by %s (%s).',
                (string) ($_SESSION['user']['fullname'] ?? 'Owner'),
                (string) ($shop['shop_name'] ?? 'Unnamed shop')
            );
            notify_sys_admins_owner_review_needed($pdo, $adminNotification, 6);

            $success = 'Business information submitted successfully. Your shop is now awaiting admin approval.';
