...
// Get shop details
$shop_stmt = $pdo->prepare("SELECT * FROM shops WHERE owner_id = ? ORDER BY id DESC LIMIT 1");
...
$shop_id = $shop['id'];
$active_assignment_statuses = order_workflow_active_statuses();
$active_assignment_placeholders = order_workflow_status_placeholders($active_assignment_statuses);
...
if(isset($_GET['deactivate'])) {
    ...
    if($staff) {
        ...
        $unassign_stmt = $pdo->prepare("
            ...
            WHERE shop_id = ? AND assigned_to = ? AND status IN ($active_assignment_placeholders)
        ");
        $unassign_stmt->execute(array_merge([$shop_id, $staff['user_id']], $active_assignment_statuses));
        $unassigned_count = (int) $unassign_stmt->rowCount();
        ...
        if(function_exists('log_audit')) {
            log_audit(
                ...
                ['status' => 'active'],
                ['status' => 'inactive'],
                ['shop_id' => $shop_id, 'staff_user_id' => (int) $staff['user_id'], 'unassigned_orders' => $unassigned_count]
            );
        }
    ...
    if($staff) {
        ...
        if(function_exists('log_audit')) {
            log_audit(
                ...
                ['status' => 'inactive'],
                ['status' => 'active'],
                ['shop_id' => $shop_id, 'staff_user_id' => (int) $staff['user_id']]
            );
        }
...
// Use latest id per user/shop so legacy rows with NULL created_at are still listed.
$staffs_stmt = $pdo->prepare("
    SELECT 
        ...
        (
            ...
            WHERE o.shop_id = se.shop_id 
              ...
              AND o.status IN ($active_assignment_placeholders)
        ) as active_orders
    FROM shop_staffs se 
    ...
    JOIN (
        ...
        WHERE shop_id = ?
        GROUP BY user_id
    ) latest_staff ON latest_staff.latest_id = se.id
    WHERE se.shop_id = ? 
    ORDER BY se.created_at DESC, se.id DESC
");
// Bind active status placeholders first because they appear in the SELECT subquery
$staffs_stmt->execute(array_merge($active_assignment_statuses, [$shop_id, $shop_id]));
