...
if ($reports_table_exists && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_report'])) {
    ...
    if (!in_array($target_type, ['community_post', 'community_comment'], true) || $target_id <= 0 || $reason === '') {
        $report_error = 'Please select a valid reason before reporting content.';
    } else {
        $result = moderation_create_report($pdo, $owner_id, $target_type, $target_id, $reason, $notes);
        if (!($result['ok'] ?? false)) {
            $report_error = $result['message'] ?? 'Unable to submit report right now.';
        } else {
            $report_success = 'Report submitted for moderation review.';
        }
    }
}
           ...
           $final_price = $offer_price !== '' ? (float) $offer_price : (float) ($selected_post['preferred_price'] ?? 0);
            ...
            if ($form_error === '') {
                ...
                $order_id = order_create_unified_request($pdo, [
                    ...
                    'client_id' => (int) $selected_post['client_id'],
                    'shop_id' => (int) $shop['id'],
                    'service_type' => !empty($selected_post['category']) ? $selected_post['category'] : 'Community Request',
                    'design_description' => $selected_post['description'] ?? null,
                    'quantity' => $final_quantity,
                    'price' => $final_price,
                    'client_notes' => 'Converted from community post: ' . ($selected_post['title'] ?? 'Untitled request'),
                    'quote_status' => 'issued',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_shop_comment'])) {
    ...
    if (!$community_comments_table_exists) {
        ...
        $post_exists_stmt = $pdo->prepare('SELECT id, client_id FROM client_community_posts WHERE id = ? LIMIT 1');
        $post_exists_stmt->execute([$post_id]);
        $post_row = $post_exists_stmt->fetch();
        ...
        if (!$post_row) {
           ...
           if ($comment_offer_price_exists && $comment_offer_quantity_exists) {
            ...
            notify_in_app_participants(
                $pdo,
                [(int) ($post_row['client_id'] ?? 0)],
                null,
                'message',
                'New shop comment on your community request.',
                10,
                $owner_id
            );
...
if ($view_post_id > 0) {
    $view_post_stmt = $pdo->prepare("
        ...
        WHERE ccp.id = ? AND ccp.is_removed = 0
        LIMIT 1
    ");
...
$posts_stmt = $pdo->query("
    ...
    FROM client_community_posts ccp
    JOIN users u ON ccp.client_id = u.id
    WHERE ccp.status = 'open' AND ccp.is_hidden = 0 AND ccp.is_removed = 0
...
if ($community_comments_table_exists && !empty($community_posts)) {
    ...
    $comments_stmt = $pdo->prepare("\n        SELECT c.id,
        ...
        WHERE c.post_id IN ($placeholders) AND c.is_removed = 0
            ...
            <div class="post-grid">
                <?php foreach ($community_posts as $post): ?>
                    <div class="community-post">
                        ...
                        <?php endif; ?>
                        <p class="text-muted mb-2 post-summary"><?php echo htmlspecialchars(mb_strimwidth((string) $post['description'], 0, 95, '…')); ?></p>
                        <div class="community-meta">
                            <?php if (!empty($post['preferred_price'])): ?>
                                <span><i class="fas fa-peso-sign"></i> Est. Price ₱<?php echo number_format((float) $post['preferred_price'], 2); ?></span>
                            <?php endif; ?>
                        ...
                        <details class="comment-section">
                            ...
                            <div class="mt-2 community-meta">
                                ...
                                <span><i class="fas fa-tag"></i> <?php echo htmlspecialchars($post['category']); ?></span>
                                <?php if (!empty($post['preferred_price'])): ?>
                                    <span><i class="fas fa-peso-sign"></i> Preferred ₱<?php echo number_format((float) $post['preferred_price'], 2); ?></span>
                                <?php endif; ?>
                            ...
                            <div class="mt-2">
                                <p class="mb-2"><?php echo nl2br(htmlspecialchars($post['description'])); ?></p>
                                <?php if (!empty($post['image_path'])): ?>
                                    <img src="../assets/uploads/<?php echo htmlspecialchars($post['image_path']); ?>" alt="Post reference image">
                                <?php endif; ?>
                            </div>
                            ...
                            <?php else: ?>
                                <?php foreach ($shop_comments as $shop_comment): ?>
                                    <div class="comment-item">
                                        ...
                                        <?php if ((int) ($shop_comment['is_hidden'] ?? 0) === 1): ?>
                                            <p class="mb-2 mt-1 text-muted"><em>This comment was moderated and hidden.</em></p>
                                        <?php else: ?>
                                        ...
                                        <?php endif; ?>
                                        <?php if ($reports_table_exists): ?>
                                            <details class="mt-2"><summary class="text-danger small"><i class="fas fa-flag"></i> Report comment</summary>
                                                <form method="POST" class="mt-2">
                                                    <?php echo csrf_field(); ?>
                            ...
                            <?php if ($community_comments_table_exists): ?>
                                <details class="mt-2">
                                    <summary class="text-primary">Suggest your shop service</summary>
                                <form method="POST" class="mt-2">
                                    <?php echo csrf_field(); ?>
