...
if($existing_shop) {
    $snapshot = fetch_owner_onboarding_snapshot((int) $owner_id, $pdo);
...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        $update_stmt = $pdo->prepare("UPDATE users SET status = 'pending' WHERE id = ?");
        $update_stmt->execute([$owner_id]);

        header("Location: shop_profile.php?success=shop_created");
        exit();
