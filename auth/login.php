...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        if($user && password_verify($password, $user['password'])) {
            $canonicalRole = canonicalize_role($user['role'] ?? null);
            $canonicalStatus = canonicalize_user_status($user['status'] ?? null);

               if ($canonicalStatus !== 'active' && $canonicalRole !== 'owner') {
                ...
                } elseif (!can_role_login((string) $canonicalRole, $pdo)) {
        ...
        log_runtime_error('login_attempt', $e);
        $error = user_safe_error_message(
            'Login failed due to a temporary system issue. Please try again.',
