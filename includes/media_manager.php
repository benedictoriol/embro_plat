...
function collect_media_references(PDO $pdo): array {
    ...
    $digitized_stmt = $pdo->query("SELECT stitch_file_path FROM digitized_designs WHERE stitch_file_path IS NOT NULL AND stitch_file_path <> ''");
    foreach ($digitized_stmt->fetchAll(PDO::FETCH_COLUMN) as $file) {
        $file = (string) $file;
        $normalized = preg_replace('#^assets/uploads/#', '', $file);
        $parts = explode('/', (string) $normalized, 2);
        if (count($parts) === 2) {
